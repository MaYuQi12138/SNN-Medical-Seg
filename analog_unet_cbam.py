...
class AnalogCAM(nn.Module):
    ...
    def forward(self, x):
        ...
        t, b, c = weight.shape
        # 通道注意力Mc
        Mc = torch.reshape(weight, (t, b, c, 1, 1))
...
class AnalogSAM(nn.Module):  # 空间注意力模块
    def __init__(self, in_channel):
        super(AnalogSAM, self).__init__()
        self.conv = layer.Conv2d(2, 1, kernel_size=7, padding=3)
        self.sigmoid1 = neuron.IFNode()
        self.sigmoid2 = neuron.IFNode()
    ...
    def forward(self, x):
        ...
        # 卷积操作得到空间注意力结果
        x_cat = self.sigmoid1(x_cat)
        x_out = self.conv(x_cat)
        Ms = self.sigmoid2(x_out)
...
class AnalogCBAM(nn.Module):
    def __init__(self, in_channel):
        super(AnalogCBAM, self).__init__()

        self.Cam = AnalogCAM(in_channel=in_channel)  # 通道注意力模块
        self.Sam = AnalogSAM(in_channel=in_channel)  # 空间注意力模块
